...
% the .mot file are used to calculate muscle-tendon length in OpenSim 
...
%% calculate muscle length             
            mkdir([pwd '\MuscleAnalysis\']);
            output_path = ([pwd '\MuscleAnalysis\']);
            timeFrame =  [dataC3D.marker_data.Time(1,1) dataC3D.marker_data.Time(end,1)]; % change if needed
            OpenSim_Muscle_Analysis(MotFileName,model_ref,output_path,timeFrame)
